...
if [ -f /etc/os-release ] && grep -q 'VERSION_ID="8' /etc/os-release; then
    echo "Running on EL8 machine, launching with singularity"
    RUN_ON_SINGULARITY=1
fi
...
if [ $RUN_ON_SINGULARITY -eq 0 ]; then
    ...
    singularity run -B /cvmfs -B /eos -B /etc/grid-security -B /etc/pki/ca-trust --home $PWD:$PWD /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/cc7:x86_64 bash $SCRIPT_PATH ${@:2}
fi 
